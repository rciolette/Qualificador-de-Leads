-- O caminho lento de `filtrar` (papel sem permissão de reconstruir a foto) monta
-- o objeto por conta própria, e precisava ganhar os derivados de negócio também.
--
-- Terceiro e último lugar que monta esse objeto. Se ele ficasse de fora, um
-- `leitor` veria "Time com negócio GANHO" no seletor e o filtro não acharia
-- nada -- o campo existiria na tela e não existiria no dado.
