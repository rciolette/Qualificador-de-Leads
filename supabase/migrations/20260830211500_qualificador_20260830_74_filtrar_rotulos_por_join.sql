-- O `return query` da migration 72 resolvia o rótulo com um `left join lateral`
-- sobre `jsonb_array_elements(p_etapas)` -- que roda UMA VEZ POR PESSOA, 4.430
-- vezes, para servir as poucas linhas com `saida >= 10`.
--
-- Os rótulos são no máximo 8 + N etapas. Materializá-los uma vez numa tabela
-- minúscula e fazer um join normal troca 4.430 execuções por uma.
--
-- (Medido: 1.715 -> 1.632 ms. O ganho foi pequeno porque o gargalo real estava
-- em outro lugar -- ver a migration 75.)
--
-- NOTA: a migration 76 reescreve esta função. Arquivo mantido para o repo
-- reproduzir a ordem em que o banco mudou; o corpo final está na 76.
    ...
    create temp table _et on commit drop as
      select d.pessoa_id, d.dados,
             case when not pular_bloqueio then
               case
                 when d.dados->>'telefone_e164' is null then 1
                 when coalesce(qualificador.como_bool(d.dados->>'unsub_whats'), false) then 2
                 when coalesce(qualificador.como_bool(d.dados->>'em_cadencia_automatica'), false) then 3
                 when coalesce(qualificador.como_bool(d.dados->>'falha_sellflux'), false) then 4
                 when coalesce(qualificador.como_bool(d.dados->>'perdido_na_cadencia'), false) then 5
                 when qualificador.como_ts(d.dados->>'perdido_em')
                      > now() - make_interval(days => perdido_dias) then 6
                 when coalesce(qualificador.como_bool(d.dados->>'cadencia_iniciada'), false)
                      and qualificador.como_bool(d.dados->>'conectou') is false then 7
                 when exists (
                   select 1 from qualificador.disparo_registro dr
                    where dr.pessoa_id = d.pessoa_id
                      ...
                      and (d.dados->>'preferential_whats_id' is null
                           or dr.numero_whats = d.dados->>'preferential_whats_id')) then 8
               end
             end as saida
