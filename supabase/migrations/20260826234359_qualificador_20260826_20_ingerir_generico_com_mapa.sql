-- Qualificador de Leads ROI · migration 20
-- ingerir_generico passa a receber o mapeamento como parâmetro.
--
-- A primeira versão lia o de-para de fonte_importacao via importacao.fonte_importacao_id.
-- Isso obrigava a Edge Function a criar um "perfil anônimo" por importação só para ter
-- onde guardar o mapa, e uma importação sem perfil salvo falhava com
-- "precisa de pelo menos um identificador" mesmo tendo o mapa correto no corpo.
-- Passar o mapa direto elimina a indireção e o modo de falha junto.
--
-- O mapa usado fica gravado na própria importação: dá para saber, meses depois,
-- com que de-para aquela base entrou.

alter table qualificador.importacao
  add column mapeamento     jsonb,
  add column transformacoes jsonb;

comment on column qualificador.importacao.mapeamento is
  'De-para efetivamente aplicado nesta importação. Registro histórico: o perfil
   pode mudar depois, esta coluna não.';

create or replace function qualificador.ingerir_generico(
  p_importacao_id uuid,
  p_mapa    jsonb default null,
  p_transf  jsonb default null,
...
declare
  ...
  v_valor_centavos boolean; v_data_brt boolean;
  v_desconhecidos text[];
  v_linhas int := 0; v_pessoas int := 0; v_transacoes int := 0;
  v_sem_id int := 0; v_fora_regra int := 0;
begin
  -- o que veio na chamada manda; o perfil da importação é o fallback
  ...
  -- fica registrado com que de-para esta base entrou
...
drop function if exists qualificador.ingerir_generico(uuid);
